# Build + install the RPG Stations plugin jar in one shot.
#
# Self-contained (mirrors the ziggfreed-common / mmo-mob-scaling build.ps1): gradlew
# build, pin the exact runtime jar by version (NOT the -sources/-javadoc siblings),
# and copy it into the Hytale Mods folder when one is known.
#
...
Write-Host "`n=== Building RpgStations (gradlew build) ===" -ForegroundColor Cyan
...
# 'RpgStations-*.jar' - that also matches the -sources.jar / -javadoc.jar siblings.
