...
/**
 * The Stamp step's {@code Stats} leaf delegate: RpgStations owns the roll +
 * cap MATH (weighted pick, {@code Picks}/{@code Unique}, cap composition), but never
 * the item-format write - a registered stamper is the ONE thing that knows how "N points of stat
 * X" gets encoded onto a stack. Deliberately a thin, format-agnostic 2-method contract rather than
 * a richer context object: RpgStations never needs anything back from the stamper except these two
 * reads/writes.
 *
 * <p>No stamper registered = the {@code Stats} leaf no-ops entirely (a validator/runtime-audit
 * warning fires once) while {@code Durability} still lands - a standalone anvil with no other mod
 * installed stamps native per-stack durability only.
 */
public interface EnhanceStamper {

    /**
     * Read-only: {@code stack}'s enhancement state BEFORE this stamp attempt. Called by
     * {@code station.StampCapEngine} during the Stamp step's COMPUTE phase (zero mutation
     * there); must not throw for a bare/never-enhanced stack ({@link StampInspection#empty()}
     ...
     * caller and the whole step fails with custody restored to its exact pre-step contents.
     * RpgStations never inspects the returned lines' meaning, it renders their provider-composed
     * labels verbatim.
     */
    @Nonnull
