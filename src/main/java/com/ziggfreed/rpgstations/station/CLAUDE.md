...
`firstRunnableConversionFromCustody` already build, never a second matching rule, and every one of
the four places `cyclesDone` can increment pushes it behind a before/after compare (a suspended
