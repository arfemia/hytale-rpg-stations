...
Router for `command/`. One admin-gated command group.
...
- **[`RpgStationsCommand`](RpgStationsCommand.java)** is `/rpgstations <camera|validate> [action]`,
  ...
  level (a declared admin group, not a manual runtime `hasPermission` check; this mod's own
  `util.Permissions` stays unused by this command, reserved for a future finer-grained check).
  - `camera <preset>` - set the CALLING player's own [`StationCameraPreset`](../station/StationCameraPreset.java)
    ...
    never persisted) for their NEXT station session.
  ...
  - `validate` - run `station.StationValidator.validate()` over the folded station/lootable
    ...
    `StationValidator.runAndLog()` (called again here so the log carries a matching run: chat a
    live run, log a matching one).
