...
`MaxCustodyClaimsPerWorld`, three independent nullable per-world ceilings, unlimited by default);
a `RemoveWorldEvent` listener (`RpgStationsPlugin#registerWorldEviction` ->
`StationService#onWorldRemoved`) that finally EVICTS this engine's global block-keyed maps by
world-uuid prefix on world unload (they never partitioned per world, so an instance-world fleet
leaked a stale entry per station block for the whole uptime); a disconnect claim-eviction sweep
(`StationService#returnClaimsOf`/`#claimWorldsOf`) that hands back placed input with no live
session behind it (the one shape none of the four pre-existing claim-removal paths ever covered);
